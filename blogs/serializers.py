...
class BlogSerializer(serializers.ModelSerializer):
    ...
    This serializer handles the serialization and deserialization of Blog
    objects, including additional computed fields for ownership, profile
    information, and like status.
    """
    ...
    def validate_banner(self, value):
        """
        Validate the banner image to ensure it meets the size and dimension
        requirements.
        ...
        if value.size > max_size:
            raise serializers.ValidationError(
                'Image size is larger than 2 MB, please try uploading a '
                'smaller image.'
            )
        ...
        if value.image.height > max_height:
            raise serializers.ValidationError(
                f'Image height is larger than {max_height} pixels, please '
                'upload a smaller image.'
            )
    ...
    def validate_image(self, value):
        """
        Validate the content image to ensure it meets the size and dimension
        requirements.
    ...
    def get_blog_like_id(self, obj):
        """
        Get the ID of the 'BlogLike' instance if the current user has liked the
        given blog.
        ...
        Returns:
            int or None: The ID of the 'BlogLike' instance if the current user
            has liked the blog, None otherwise.
        """
